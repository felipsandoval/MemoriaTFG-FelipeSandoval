...
La experimentación para este proyecto se ha desarrollado a modo de ensayo y error. Contaba con una serie de 200 ficheros para corroborar la correcta detección de las duplicidades. Al ejecutar la herramienta en estos ficheros se obtuvieron resultados satisfactorios, mostrando por pantalla la información de cada proyecto de manera satisfactoria.

Se ejecuta sobre el test.sb3

Se creaban programas en scratch y se comprueba que los resultados que muestra el programa son verídicos.

Para analizar la funcionalidad de mi código se crean los ficheros \textit{test\_program.py}, \textit{test\_DuplicateScripts.py} y \textit{test\_most\_frequent\_block.py}
...
\item Extender la funcionalidad de reconocimiento de bloques duplicados para reconocer la duplicidad de los \textit{custom blocks}, tanto a nivel \textit{intra-sprite} como \textit{proyect-wide}.
