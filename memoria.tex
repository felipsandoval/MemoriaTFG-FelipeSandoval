...
La herramienta se desarrolla usando Python como principal tecnología. Durante el trabajo se realizarán tests unitarios y guías de estilo para mejorar la calidad del código. Por ultimo, se explicarán los resultados y analizarán, concluyendo si se consigue o no, los objetivos planteados.
...
Para poder analizar la duplicidad de bloques de código en Scratch es necesario extraer la información del proyecto. Todos los datos de variables, objetos y bloques están contenidos en el fichero JSON dentro de la extensión SB3 que usa Scratch. Una vez se obtiene esta información es necesario procesar, según una serie de parámetros, los bloques que más se repiten. Luego, el análisis de dichos datos se realiza mediante la asignación de familias de grupos entre puntos de datos, mejor conocido como algoritmo de propagación por afinidad. Se emplea este algoritmo ya que considera cada punto de datos como un nodo en una red que recursivamente itera hasta tener un buen conjunto de ejemplares para clasificar grupos.\cite{clusteringpaper}
...
Este módulo implementa tipos de datos que proporcionan alternativas a los contenedores integrados de uso general de Python, como diccionarios, listas, set, y tuplas.\cite{collectionWeb}
...
La propagación de afinidad toma como entrada una colección de similitudes de valor real entre puntos de datos, donde la similitud s(i,k) indica
...
En clustering no existe información a priori de la pertenencia de los patrones a un conjunto de clases predefinida. Esto representa un problema en la evaluación de los resultados obtenidos a través de diferentes métodos.

\cite{wing_socialissues}
...
\begin{itemize}
\item La importancia del desarrollo computacional. Al amar tanto la programación me he podido percatar de la importancia del temprano desarrollo de estas habilidades en niños y niñas. Para que así puedan programar blabla
\item El lenguaje de programación Scratch.
\item La utilidad de las pruebas unitarias. Unittesting
\item La facilidad de \LaTeX.
\item Algoritmos de clustering en python. 
\end{itemize}
...
El software tiene un amplio rango de mejoras tanto a nivel de código como de funcionalidades. Se sugieren las siguiente implementaciones:

\begin{itemize}
\item Ampliar las pruebas haciendo uso de una base de datos con múltiples proyectos de Scratch.
\item Dotar de feedback al usuario donde se le indique otras posibles combinaciones de sus bloques más duplicados.
\item Mostrar gráficas dónde se muestren los bloques que mas suelen duplicarse.
\item Crear una interfaz gráfica para facilitar su interacción y uso. Se podría desarrollar una aplicación web interactiva alojada en GitLab.
\item Desarrollar la funcionalidad para reconocer duplicidad de \textit{custom blocks} tanto a nivel \textit{intra-sprite} como \textit{proyect-wide}.
\item Integrar el software en la aplicación de Dr. Scratch. Esto habilitaría la posibilidad de obtener la cuantía sobre la duplicidad de código y puntuarlo. 
\item Ampliar las pruebas con otros algoritmos de clustering. Se podría realizar un trabajo investigativo donde se compare la efectividad de varios algoritos, tanto de aprendizaje supervizado como aprendizaje no supervisado. Esto serviría para confirmar si efectivamente el algoritmo de propagación por afinidad es el más óptimo.
\end{itemize}
