...
En este trabajo se busca desarrollar una herramienta capaz de extraer, estudiar y analizar la duplicidad de código en proyectos de Scratch. Durante el proceso, se analizan los distintos llamados \textit{bloques} que conforman cada proyecto y se obtiene como resultado una lista con aquellos que mayor duplicidad presenten. Para lograr este objetivo se realiza un estudio estadístico tanto a nivel \textbf{intra-sprite} como a nivel \textbf{intra-project} haciendo uso de algoritmos de clustering.

Esta idea surge a partir de otras herramientas que evalúan aspectos del pensamiento computacional, el cual abarca habilidades como el racionamiento lógico, la sincronización, el paralelismo, la representación de la información y la abstracción. Es esta última habilidad la que motiva la ejecución de este trabajo.
...
\begin{enumerate}
  	...
  	\item \textbf{Estado del arte:} En este capítulo se describen las tecnologías que se han implementado en el trabajo y se detallan conceptos para entender su estructura.
  	\item \textbf{Diseño e implementación:} En este capítulo se visualiza el funcionamiento del programa, desde la arquitectura que se sigue hasta el funcionamiento de las funciones y el hilo de ejecución completo.
  	\item \textbf{Experimentos, validaciones y resultados:} En este capítulo se profundiza en las pruebas realizadas, en los métodos de validación para el funcionamiento del código y los resultados obtenidos.
...
\begin{itemize}
  	\item Estudiar la estructura visual y funcional del lenguaje de programación Scratch, especialmente el fichero con formato JSON\footnote{\url{https://en.scratch-wiki.info/wiki/Scratch_File_Format}} de cada proyecto Scratch.
...
A continuación, se puede visualizar en la figura \ref{fig:diagrama_gantt} el cronograma de elaboración en un diagrama de Gantt.

\begin{figure}[h]
	\centering
    \includegraphics[width=16cm, keepaspectratio]{img/gantt.jpg}
    \caption{Cronograma de realización del proyecto}
